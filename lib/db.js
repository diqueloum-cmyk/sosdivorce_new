...
export async function getAllPaidSessions(limit = 50, offset = 0, paidOnly = false) {
  try {
    ...
    if (paidOnly) {
      result = await sql`
        SELECT ps.*,
               (SELECT COUNT(*) FROM paid_messages pm WHERE pm.session_id = ps.id AND pm.role = 'user') as message_count
...
export async function getAllUnpaidSessions(limit = 50, offset = 0) {
  try {
    const result = await sql`
      ...
      LEFT JOIN unpaid_messages um ON us.id = um.session_id AND um.role = 'user'
