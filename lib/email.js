...
function generateAnalysisEmailHTML({ expertiseName, amountEuros, questionnaireData, messages, isPremium, clientEmail, paymentIntentId }) {
  ...
  // Trouver le message d'analyse (généralement un des derniers messages de l'assistant)
  let analysisContent = '';
  for (let i = assistantMessages.length - 1; i >= 0; i--) {
    const content = assistantMessages[i].content;
    if (content.length > 200 && !content.includes('[PAIEMENT_PRET]')) {
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="padding: 0 30px;">
              <div style="background-color: #f0fdf4; border-left: 4px solid #22c55e; border-radius: 0 8px 8px 0; padding: 20px; margin-bottom: 20px;">
                <h3 style="color: #166534; margin: 0 0 15px 0; font-size: 18px;">Notre analyse</h3>
                <div style="color: #374151; line-height: 1.6; white-space: pre-line;">${escapeHtml(analysisContent || 'Votre analyse détaillée vous sera envoyée dans un email séparé.')}</div>
              </div>
            </td>
          </tr>
 ...
 */
function generateAnalysisEmailText({ expertiseName, amountEuros, questionnaireData, messages, clientEmail, paymentIntentId }) {
  const assistantMessages = messages.filter(m => m.role === 'assistant');
...
ANALYSE
-------------
${analysisContent || 'Analyse détaillée à compléter.'}
